...
contract = "Returns the larger of the two input values."
notes = """
...
static void bh_tests(void)
{
    ...
    BH_ASSERT_OPT_EQ(((bh_input_t){ .x = INT64_C(1), .y = INT64_C(2) }), INT64_C(2));
    BH_ASSERT_OPT_EQ(((bh_input_t){ .x = INT64_C(2), .y = INT64_C(1) }), INT64_C(2));
    BH_ASSERT_OPT_EQ(((bh_input_t){ .x = INT64_C(-1), .y = INT64_C(1) }), INT64_C(1));
    BH_ASSERT_OPT_EQ(((bh_input_t){ .x = INT64_C(1), .y = INT64_C(-1) }), INT64_C(1));
    BH_ASSERT_OPT_EQ(((bh_input_t){ .x = INT64_MIN, .y = INT64_MAX }), INT64_MAX);
    BH_ASSERT_OPT_EQ(((bh_input_t){ .x = INT64_MAX, .y = INT64_MIN }), INT64_MAX);
