...
def extract_readme_sources() -> list[str]:
    text = README_MD.read_text(encoding="utf-8")
    match = re.search(r"(?ms)^## Sources\s*$\n(.*?)(?:^\#|\Z)", text)
    if match is None:
        raise ValueError("README.md is missing a Sources section")
    ...
    if not sources:
        raise ValueError("README.md Sources section contains no links")
    return sources
...
def render_index(records: list[dict], sources: list[str]) -> str:
    lines = [
        "---",
        'layout: default',
        'title: Bit Hacks',
...
def main() -> None:
    ...
    readme_sources = extract_readme_sources()
    records = []
    all_tags = set()
    ...
    INDEX_MD.write_text(render_index(records, readme_sources), encoding="utf-8")
